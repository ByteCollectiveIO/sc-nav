...
Built as the marketplace's **fourth listing mode** (`commission`), reusing the
listings/offers/dual-confirm machinery wholesale; the requester stays `seller_id`
("owner") so every permission path works unchanged. The new substance is the
**blueprint reference feed**, synced from the **SC Wiki open API**
(`api.star-citizen.wiki/api/blueprints`, 1,559 recipes, game-version-stamped;
the local mine in `docs/datamined_data/blueprints_data/` is the cross-check —
it's missing all gem/item-kind ingredients): materials manifest (resources ×
SCU + gems × count × min-quality), craft time, unlock missions, and per-aspect
quality→stat modifier ranges that power a spec-builder UI with per-aspect
sliders ("Damage Mitigation rides the Shell / Stileron quality"). Sync →
committed `poi/blueprints.json` per the quantum-pipeline convention; `/api/blueprints` index + detail;
`blueprint:` catalog namespace; two `_ensure_column`s on `listings`
(`blueprint_key`, `materials`); spec stored in the existing `attributes` blob.
v1.1 = `member_blueprints` library → "requests I can craft" matching. Deferred:
estimated material cost off raw-commodity prices, structured quotes,
inventory-bridge for requester-supplied materials.
