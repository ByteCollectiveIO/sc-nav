...
| Doc | Status | Covers |
|---|---|---|
| [blueprint-readiness.md](blueprint-readiness.md) | 🔨 built 2026-09-12 · unreleased (rides the #151 hold) | **Org blueprint readiness + unlock goals**: every feed recipe w/ holders + unlock path (`GET /api/blueprints/readiness`, Blueprints › Org readiness), select → **unlock goal** (`goals.kind=unlock`, `unlock_spec` {blueprints, target count|pct, playstyle}) whose progress counts scope MEMBERS holding each recipe (`nav_core.derive_unlock_progress`); library adds flip it met + goals-channel ping |
| [blueprint-library.md](blueprint-library.md) | 🔨 built 2026-09-12 · unreleased (rides the #151 hold) | **#/blueprints as a real table**: category groups, sortable Recipe/Maker/Size·Grade/Materials/Est. cost/Time/Craftable, text+category+material filters, row expand = free stock vs one craft + expected stats. UEX item spec joined by name (maker/size/class/grade, 1,493/1,559); `nav_core.craftable_from_holdings` fills each slot best-quality-first from FREE stock → ok/crafts/weakest-input quality; **List** seeds a marketplace sale with the recipe, craft count, that quality and the expected stats |
| [inventory-quality.md](inventory-quality.md) | ✅ all 5 steps built 2026-09-12 · dev-server test pending, unreleased | #151: **quality on inventory holdings** — the holding key `(owner, item, location)` overwrites a second quality lot; 4.10 keeps Q1–1000 on commodities and stacks per value. Adds nullable `quality` (null = unrated, 0 = station-bought) on commodity lots + the marketplace `CraftedIn` blob on crafted holdings, derived `subkind`/band, then threads it: goal progress gates on `min_q` (`have_low` bucket, `allow_low` override, manual floors for contract hand-ins), craft-goal preview on pledged lots, commodity-quality listings + list-from-inventory, band-grouped org rollup. Settled: strict gating w/ `allow_low`, edit-in-place only while unallocated else split, band derived not stored, null backfill + UNIQUE index w/ logged collisions, blank-not-0 form default |
| [trade-topup.md](trade-topup.md) | 🔨 PR 0–PR 2 built (PR 2 mixed loads 2026-08-23), awaiting live test | Trade planner mid-run **top-up** + **mixed loads**: a shorted buy leaves a big hold empty — run mode grows `leg.extras` co-cargo lots (same buy POI, same sell POI), a `⊕ top up from here` suggestion panel (`GET /api/trade/run/topup`), add-as-bought `addcargo`, per-lot sells, multi-held re-plan; PR 2 = plan-time `cargo: mixed` bundle candidates (superset pool; planned lots ride `leg.lots` and convert via the existing `addcargo`) |
...
| [survey-zones-surface.md](survey-zones-surface.md) | 📐 design | Surface survey zones (#37.1, the doc `survey-platform.md` §7.2 asked for): named 50 km mining areas on planets/moons, membership **geometric + retroactive** over existing observations (no capture change, no backfill), same `survey_zones` table + nullable surface anchor → one ATLAS list + one detail card; navigable via nearest QT marker, never a drop target; three-level map (body badge → equal-area body plate → local zone plate); **passive contribution** — normal mining fills a zone with no tagging or awareness, + `kind="survey"` priority goals with a `since` baseline |
