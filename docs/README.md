...
| Doc | Status | Covers |
|---|---|---|
| [trade-topup.md](trade-topup.md) | 🔨 PR 0 + PR 1 built 2026-08-22 · PR 2 designed | Trade planner mid-run **top-up** + **mixed loads**: a shorted buy leaves a big hold empty — run mode grows `leg.extras` co-cargo lots (same buy POI, same sell POI), a `⊕ top up from here` suggestion panel (`GET /api/trade/run/topup`), add-as-bought `addcargo`, per-lot sells, multi-held re-plan; PR 2 = plan-time `cargo: mixed` bundle candidates (designed, not built) |
