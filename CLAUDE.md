...
- Halo Finder (#31, docs/halo-finder.md): `GET /api/halo/bands` (static Cornerstone band table + attribution), `POST /api/halo/plan` (`HaloPlanIn`: `band` 1–10 XOR `target_poi_id`; `start_poi_id` or live fix; `aim` band|peak; `allow_staging`; ship/qd; shared `avoid_poi_ids`) → `{start, band|target, staged, legs, drop, alternates, attribution}` (alternates = full drop+leg pairs, client promotes without re-plan), `GET /api/halo/locate` (classify latest fix: band / band_offplane / void / outside; `?target_poi_id` adds miss distance — the Refine loop). Solver `nav_core.plan_halo_drop` + geometry (`HALO_BANDS` constants, `_ring_crossings`, `halo_band_crossings`, `halo_locate`, `halo_contains` = ring-envelope test so `system_at` resolves an in-belt deep-space fix to Stanton, not a coincidentally-near Pyro/Nyx body (v0.52.1), `body_volumes` + `chord_obstructed` endpoint-aware sun/planet obstruction — low-orbit/surface starts stay plannable, v0.51.1) at the end of nav_core.py; capture flow band-annotates deep-space POIs (`_halo_capture_note`); navigator deep-space chip `haloWhereChip`. **HALO MAP (v0.52.0):** plan carries `start.xyz`/`target.xyz`/`drop.marker_xyz`/staging `to_xyz`, bands feed carries `bodies` (star + true planets) → `drawHaloMap`/`drawHaloInset` canvas in the plan card (true-scale system view + magnified drop-zone inset, fixed dark scope colors like the navigator map)
