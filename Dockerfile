...
# The vendored starmap snapshots (containers, POI catalog, tombstones for
# ghost-anchor recovery) are code-bundled too: existing /data volumes hold a
# stale pre-vendoring fetch cache that the code-dir copy must shadow
# (app._static_feed / app.load_shipped_containers read code-dir first).
COPY poi/quantum_drives.json poi/quantum_profiles.json poi/blueprints.json poi/locations.json poi/containers.json poi/poi.json poi/container_tombstones.json ./
